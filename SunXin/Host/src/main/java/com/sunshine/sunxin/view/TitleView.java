...
public class TitleView extends LinearLayout implements View.OnClickListener {
    ...
    private long mHit[] = new long[5];
    private static int HIT_TIME = 1000;
...
//        }
    ...
    public void onClick(View view) {
        ...
        if (mHit[0] >= SystemClock.uptimeMillis() - HIT_TIME) {
            ...
            Toast.makeText(getContext().getApplicationContext(), "恭喜你中奖了", Toast.LENGTH_SHORT).show();
            for (int i = 0; i < mHit.length; i++) {
                mHit[i] = 0;
            }
        }else {
            for (int i = 0; i < mHit.length; i++) {
                if(mHit[i]>0&&mHit[i] < SystemClock.uptimeMillis() - HIT_TIME){
